...
const configSQLServer1 = require('./configServerUAT-1');
const configSQLServer2 = require('./configServerUAT-2');
...
router.get("/", (req,res) => {
    try {
        res.status(200).send({
            mensaje: "API de Servicio UAT Sql-2"
        });

    }catch(err){
        res.status(500).send({
            mensaje: "Error en Servicio UAT Sql-2"
        });
    };
});

// BUSCAR Marca POR CÓDIGO
router.post("/SQL-BuscarMarcaCod-UAT", async (req,res) => {
    try {
...
router.post("/SQL-BuscarMarcaDes-UAT", async (req,res) => {
    try {
...
router.post("/SQL-RegistroMarca-UAT", async (req,res) => {
    try {
...
router.post("/SQL-ModificarMarca-UAT", async (req,res) => {
    try {
        // Declaracion de varaible que contiene la conexión pool
        let pool = null;
...
router.post("/SQL-EliminarMarca-UAT", async (req,res) => {
    try {
        // Declaracion de varaible que contiene la conexión pool
        let pool = null;
